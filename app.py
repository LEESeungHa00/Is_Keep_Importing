...
if uploaded_file:
    ...
    # --- 3. 롤링 윈도우(Rolling Window) 기반 기간 구하는 로직 ---
    curr_end = today
    period_type = "Year"
    ...
    # 상단 기간 요약 박스 (일수가 동일하게 매칭됨)
    st.markdown("#### ⏳ 분석 기준 기간")
    col1, col2 = st.columns(2)
    col1.info(f"**최근 기간 (Current):** {curr_start.strftime('%Y-%m-%d')} ~ {curr_end.strftime('%Y-%m-%d')} ({curr_days}일)")
    col2.info(f"**직전 비교 기간 (Past):** {past_start.strftime('%Y-%m-%d')} ~ {past_end.strftime('%Y-%m-%d')} ({past_days}일)")
    ...
    if not result_df.empty:
        ...
        # Altair를 이용한 완벽한 계단식 바 차트 생성 (sort='-y' 옵션 적용)
        chart = alt.Chart(chart_df).mark_bar(color="#ff4b4b").encode(
            x=alt.X('Raw Importer Name:N', sort='-y', title='수입 업체명', axis=alt.Axis(labelAngle=-45)),
            y=alt.Y('Volume Decrease:Q', title='감소량'),
            tooltip=['Raw Importer Name', 'Volume Decrease']
        ).properties(
            height=350
        )
        st.altair_chart(chart, use_container_width=True)

        st.markdown("#### 📋 상세 리스트 (단절 업체 우선 정렬)")
        ...
        st.success("조건에 맞는 수입 감소/단절 업체가 없습니다. 대단하네요! 🎉")
else:
